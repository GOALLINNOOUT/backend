...
router.post('/signup', async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ error: 'This email is already registered. Please use a different email or log in.' });
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({ error: 'No account found with that email. Please sign up.' });
    }
    ...
    if (user.status === 'suspended' || user.status === 'blacklisted') {
      return res.status(403).json({ error: 'Your account is currently suspended. Please contact support for assistance.' });
    }
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    return res.json({ message: 'If this email is registered, we have sent a password reset link to your inbox.' });
  } catch (err) {
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (!token || !password) return res.status(400).json({ error: 'Invalid request. Please use the password reset link sent to your email.' });
...
router.post('/setup-password', async (req, res) => {
  try {
    const { token, password } = req.body;
    if (!token || !password) return res.status(400).json({ error: 'Invalid request. Please use the setup link sent to your email.' });
    if (password.length < 6) return res.status(400).json({ error: 'Your new password must be at least 6 characters long.' });
    const payload = verifySetupToken(token);
    if (!payload) return res.status(400).json({ error: 'This setup link is invalid or has expired. Please request a new one.' });
